...
namespace {
  ...
  struct Optimize : public ModulePass {
    ...
    bool IfCommute(FnCall gateA, FnCall gateB);
...
bool Optimize::IfCommute(FnCall gateA, FnCall gateB)
{
    //Check if two gates are on different qubits
    unsigned shrCount = 0;  //# of qubits/operands in common
    /*encoding shared qubit, false:if it is a target, true: if it is a control
     *{first shared qbit for A, first shared qbit for B, second for A, second for B}}
     */
    bool shrInd[4] = {0, 0, 0, 0}; 
    for(unsigned aIndex = 0; aIndex < gateA.qArgs.size(); aIndex++)
    {
        for(unsigned bIndex = 0; bIndex < gateB.qArgs.size(); bIndex++)
        {
            if(gateA.qArgs[aIndex].argPtr == gateB.qArgs[bIndex].argPtr)
            {
                bool dimCheck = true;
                for(int ndim = 0; ndim < gateA.qArgs[aIndex].numDim; ndim++)
                    if(gateA.qArgs[aIndex].dimSize[ndim] != gateB.qArgs[bIndex].dimSize[ndim])
                    {
                        dimCheck = false;
                        break;
                    }
                if(dimCheck){
                    shrCount++;
                    shrInd[2*shrCount-2] = aIndex;
                    shrInd[2*shrCount-1] = bIndex;
                }
            }
        }
    }

    if(!shrCount) return true;
    
    //two CNOT commute if the target of one is not the control of the other
    if(gateA.func->getName().find("CNOT") && gateB.func->getName().find("CNOT"))
    {
        if(shrCount == 1){
            return (shrInd[0] == shrInd[1]);
        } 
        if(shrCount == 2){
            return (shrInd[0] == shrInd[1]) && (shrInd[2] == shrInd[3]);
        }
    }

    if((gateA.func->getName().find("CNOT") && gateB.func->getName().find("T.")) ||
       (gateA.func->getName().find("T.") && gateB.func->getName().find("CNOT")) ||
       (gateA.func->getName().find("CNOT") && gateB.func->getName().find("S.")) ||
       (gateA.func->getName().find("S.") && gateB.func->getName().find("CNOT")))
    {
        return (shrInd[0] == shrInd[1])
    }
    ...
    for( vector<Function*>::iterator it = qFuncs.begin(); it != qFuncs.end(); it++){
        if((*it)->getName() == "main")   //assuming after flattening, the modules left are Toffoli and main, only care about main.
        {
            ...
            for(unsigned mIndex=0;mIndex<mapFunction.size();mIndex++){
                ...
                if(mIndex!=mapFunction.size()-1)
                    errs()<<"Commute?:"<<IfCommute(mapFunction[mIndex],mapFunction[mIndex+1])<<"\n";
            }
        }
        /*
